...
function Watchlist({ t, lang, list, setList, history, setHistory, onOpen }) {
  ...
  const recheckOne = async (ticker) => {
    ...
    try {
      ...
      setList((prev) => {
        const old = prev.find((it) => it.ticker === ticker);
...
function MarketScan({ t, lang }) {
  ...
  const scan = async () => {
    ...
    try {
      ...
      setShortList(top.short || []);
      setLongList(top.long || []);

      setStage(t.scanUpStage);
      const upText = await askClaude(
        `Today is ${today}. Use web search for the latest market news, sector momentum, earnings beats, upgrades and catalysts. List up to 50 publicly traded stocks most likely to RISE near-term. Respond ONLY with ultra-compact JSON, no markdown: {"list":[{"t":"TICKER","r":"reason max 6 words"}]} Keep reasons extremely short so all entries fit.`
      );
      setRisers(parseJSON(upText).list || []);

      setStage(t.scanDownStage);
      const downText = await askClaude(
        `Today is ${today}. Use web search for the latest downgrades, missed earnings, lawsuits and negative catalysts. List the 10 publicly traded stocks most likely to FALL near-term. Respond ONLY with ultra-compact JSON, no markdown: {"list":[{"t":"TICKER","r":"reason max 8 words"}]}`
      );
      setFallers(parseJSON(downText).list || []);
    } catch {
      setError(t.errScan);
    } finally {
      setLoading(false);
